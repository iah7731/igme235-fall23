...
function startGame(){
    // Clicking the button calls startGame() function startGame(){
...
function fireBullet(e){
    ...
    // let mouseY = e.clienty - rect.y; // console.log(${mouseX}, ${mouseY}); 
    if (paused) return;
    ...
    if(score >= 5)
    {
        ...
        for(let i = 0; i < 3; i++)
        {
            let b = new Bullet (0xFFFFFF, ship.x + offset, ship.y); 
            bullets.push(b);
...
function loadSpriteSheet() {
    ...
    for (let i=0;i<numFrames; i++){
        ...
        let frame = new PIXI.Texture(spriteSheet, new PIXI.Rectangle(i*width, 64, width, height)); textures.push(frame);
    }
    return textures;
}

function createExplosion(x, y, frameWidth, frameHeight) {
    // http://pixijs.download/release/docs/PIXI.Animated Sprite.html // the animation frames are 64x64 pixels
    let w2 = frameWidth / 2;
